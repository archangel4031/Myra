...
# Deletes Binaries, Intermediate, DerivedDataCache folders + all *.sln and *.slnx files
...
Write-Host "   • All '*.sln' and '*.slnx' files (Visual Studio solution files)" -ForegroundColor Yellow
...
Write-Host "`n🗑️  Deleting all *.slnx files..." -ForegroundColor Green

# Delete all .slnx files recursively
Get-ChildItem -Path . -Recurse -File -Filter "*.slnx" |
    ForEach-Object {
        Write-Host "   Deleting file: $($_.FullName)" -ForegroundColor DarkGray
        Remove-Item -Path $_.FullName -Force -ErrorAction SilentlyContinue
    }
